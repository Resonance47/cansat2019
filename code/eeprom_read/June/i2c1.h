...
//===============================================================================================
#define SDA			6		// ножка данных, SDA
#define SCL			7		// ножка тактирования, SCL
...
volatile unsigned char i2c_frame_error1=0; // если > 0, значит произошла фрейм-ошибка (после STOP-команды, линии SDA и SCL не перешли в состояние HIGH, не подтянулись к + питания)
...
void i2c_stop_cond1 (void)	{
	...
	if ((I2C_PIN & _BV(SDA)) == 0) i2c_frame_error1++;	// проберяем, чтобы на ноге SDA была лог.1, иначе выдаём ошибку фрейма
	if ((I2C_PIN & _BV(SCL)) == 0) i2c_frame_error1++;	// проберяем, чтобы на ноге SCL была лог.1, иначе выдаём ошибку фрейма
	I2C_DELAY
	I2C_DELAY
	I2C_DELAY
	I2C_DELAY
