...
void flashwrite(uint32_t num_pack, int x, int y, int z, int temp, int hum, int high, int v, uint32_t lat, uint32_t lon/*рез-т работы доп мисии*/, uint32_t relat, uint32_t relon /*настоящие координаты*/, uint16_t vero1, uint16_t vero2, uint16_t vero3, uint16_t vero4, int alpha)
{
	num_pack*=40;
	...
	flash_write_1byte(num_pack+38, (uint8_t)(alpha >> 8)); flash_write_1byte(num_pack+39, (uint8_t)alpha);
}

void flashread()
{
	char FA[10];
	for(uint64_t i(0); i<16777216; i+=40)
	///for(uint64_t i(0); i<76; i+=38)
	{
		...
		sprintf(FA, "%d;", ( ( ( (int)flash_read_1byte(i+36) ) << 8) | ((int)flash_read_1byte(i+37) )) ); uart_tr_string(FA);
		sprintf(FA, "%d;\n", ( ( ( (int)flash_read_1byte(i+38) ) << 8) | ((int)flash_read_1byte(i+39) )) ); uart_tr_string(FA);
	}
}
